...
def avg_data_2013():
    temp_i=0 
    average=[]
    for rows in pd.read_csv('Data/AQI/aqi2013.csv',chunksize=24): # take rows in data,get 24 records in row
        add_var=0
        avg=0.0
        data=[]
        df=pd.DataFrame(data=rows)
        for index,row in df.iterrows(): #iterate 24 rows
            data.append(row['PM2.5']) #appened
        for i in data:
            if type(i) is float or type(i) is int: #what every type adding
                add_var=add_var+i
            elif type(i) is str: #string cant add
                if i!='NoData' and i!='PwrFail' and i!='---' and i!='InVld':
                    temp=float(i) #so convert to float
                    add_var=add_var+temp
        avg=add_var/24 #get avg
        temp_i=temp_i+1
        
        average.append(avg) #add in list
    return average
